...
manifest plus the human authority and decision record.
...
run `blender-verify-promotion` before considering the change reproducible. The
session-contained promotion sandbox is proof of mechanism, not authorization
to modify the asset's authoritative contract.
