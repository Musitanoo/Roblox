...
SCHEMA = "https://roblox-top1.local/schemas/full-evidence.schema.json"
TERRITORIES = (
...
def capture(output: Path) -> dict[str, Any]:
    ...
    expected_full_render_count = render_matrix["modes"]["full"]["expectedRenderCount"]
    territory_evidence: list[dict[str, Any]] = []
    issues: list[str] = []
    for territory_id in TERRITORIES:
        report_path = ROOT / "build" / territory_id / "build-report.json"
        report_issues = validate_report(report_path, require_complete=True)
        if report_issues:
        ...
        if report["renderMode"] != "full" or report["evidenceCompleteness"] != "FULL_COMPLETE":
            issues.append(f"{territory_id}: build report is not complete full evidence")
        if (
            report["expectedRenderCount"] != expected_full_render_count
            or report["actualRenderCount"] != expected_full_render_count
        ):
            issues.append(
                f"{territory_id}: expected exactly {expected_full_render_count}/"
                f"{expected_full_render_count} full renders"
            )
        if not report["renderEvidenceComplete"] or report["renderEvidenceCapped"]:
            issues.append(f"{territory_id}: full render evidence is incomplete or capped")
        ...
        territory_evidence.append(
            {
                ...
                "renderEvidence": [
                    {"jobId": item["jobId"], "path": _qualified_path(territory_id, item["path"]), "sha256": item["sha256"]}
                    for item in report["renderEvidence"]
                ],
                "stressEvidence": [
                    {"sceneId": item["sceneId"], "count": item["count"], "path": _qualified_path(territory_id, item["path"]), "sha256": item["sha256"]}
    ...
    report_value: dict[str, Any] = {
        ...
        "artifactRetentionNote": (
            "Hashes attest the locally validated full build artifacts. The reproducible build/ directory and "
            "render images are intentionally excluded from the release ZIP."
        ),
        "issues": issues,
    }
    schema_issues = validate_with_schema(report_value, ROOT / "schemas/full-evidence.schema.json")
    if schema_issues:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Capture a package-safe attestation of complete Blender full evidence.")
    parser.add_argument("--output", type=Path, default=ROOT / "evidence/blender/full-evidence.json")
    ...
    if report["status"] != "PASS":
        ...
        return 1
    print(
        f"[PASS] Full attestation: {report['totalActualRenderCount']}/"
