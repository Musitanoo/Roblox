...
from compile_asset import add_part, render_views
...
def _mesh_geometry_sha256(mesh: bpy.types.Mesh) -> str:
    uv_layer = mesh.uv_layers.active
    geometry = {
        "vertices": [_round_vector(vertex.co) for vertex in mesh.vertices],
        "edges": [list(edge.vertices) for edge in mesh.edges],
        "polygons": [list(polygon.vertices) for polygon in mesh.polygons],
        "uv": (
            [_round_vector(loop.uv) for loop in uv_layer.data]
        ...
        objects.append(
            {
                ...
                "mesh": {
                    ...
                    "geometrySha256": _mesh_geometry_sha256(mesh),
                },
                "materials": materials,
                "modifiers": [
...
def apply_operations(
    ...
    for operation in operations:
        ...
        elif operation_type == "SET_BEVEL_WIDTH":
            before = float(operation["before"])
            after = float(operation["after"])
            declared = next(
                part for part in contract["geometry"]["parts"] if part["name"] == target
            )
            _assert_before(float(declared.get("bevel", 0.0)), before, operation)
            material = obj.material_slots[0].material
            visible = not obj.hide_render and not obj.hide_viewport
            candidate_part = deepcopy(declared)
            candidate_part["bevel"] = after
            mesh = obj.data
            bpy.data.objects.remove(obj, do_unlink=True)
            if mesh.users == 0:
                bpy.data.meshes.remove(mesh)
            replacement = add_part(candidate_part, material)
            replacement.hide_render = not visible
            replacement.hide_viewport = not visible
        else:
