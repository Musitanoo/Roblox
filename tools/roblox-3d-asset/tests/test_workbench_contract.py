...
class WorkbenchContractTests(unittest.TestCase):
    ...
    def test_bounded_bevel_change_is_source_bound(self) -> None:
        panel = next(
            part
            for part in self.contract["geometry"]["parts"]
            if part["name"] == "DEF_BarricadeSmall_PanelL"
        )
        panel["bevel"] = 0.0
        change_set = valid_change_set()
        change_set["targets"] = [panel["name"]]
        change_set["operations"] = [
            {
                "type": "SET_BEVEL_WIDTH",
                "target": panel["name"],
                "before": 0.0,
                "after": 0.04,
            }
        ]
        change_set["maximumTriangleDelta"] = 100
        validate_change_set(change_set, self.session, self.contract)
        binding = source_bindings(change_set, self.contract)[0]
        panel_index = self.contract["geometry"]["parts"].index(panel)
        self.assertEqual(
            f"/geometry/parts/{panel_index}/bevel",
            binding["jsonPointer"],
        )
        self.assertTrue(binding["sourcePromotable"])

    def test_rejects_bevel_beyond_component_depth(self) -> None:
        panel = next(
            part
            for part in self.contract["geometry"]["parts"]
            if part["name"] == "DEF_BarricadeSmall_PanelL"
        ...
        with self.assertRaisesRegex(WorkbenchError, "half the smallest"):
            validate_change_set(change_set, self.session, self.contract)

    def test_rejects_bevel_combined_with_another_operation_on_same_target(self) -> None:
        panel = next(
            part
            for part in self.contract["geometry"]["parts"]
            if part["name"] == "DEF_BarricadeSmall_PanelL"
        ...
        with self.assertRaisesRegex(WorkbenchError, "only operation"):
            validate_change_set(change_set, self.session, self.contract)
