...
def validate_operation(
    ...
    if operation_type == "SET_BEVEL_WIDTH":
        before = _number(operation, "before")
        after = _number(operation, "after")
        if before < 0 or after < 0:
            raise WorkbenchError("bevel width must remain non-negative")
        if before == after:
            raise WorkbenchError("operation before and after values must differ")
        maximum = min(float(value) for value in parts[target]["size"]) / 2.0
        if after >= maximum:
            raise WorkbenchError(
                "bevel width must remain below half the smallest component dimension"
            )
        if abs(after - before) > 0.1:
            raise WorkbenchError("bevel width delta exceeds the 0.1 stud bound")
        return
...
def validate_change_set(
    ...
    bevel_targets = {
        operation["target"]
        for operation in operations
        if operation["type"] == "SET_BEVEL_WIDTH"
    }
    for target in bevel_targets:
        if sum(operation["target"] == target for operation in operations) != 1:
            raise WorkbenchError(
                "SET_BEVEL_WIDTH must be the only operation for its target"
            )
...
def source_bindings(
    ...
    for operation in change_set["operations"]:
        ...
        if operation_type == "SET_COMPONENT_DIMENSION":
            ...
            pointer = f"/geometry/parts/{part_indexes[target]}/bevel"
        else:
            pointer = None
        bindings.append(
            {
